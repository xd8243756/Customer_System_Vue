      ...
      <div class="card-header">
        ...
        <div class="filters">
          ...
          <select v-model="searchCase.客訴類型" class="select">
            <option>全部類型</option>
            <option v-for="value in complaintTypes" :key="value.issueTypeId":value="value.issueTypeId">{{ value.issueTypeName }}</option>
            <!-- <option>物品損壞</option>
            <option>遲到</option>
          ...
          <select v-model="searchCase.處理狀態" class="select">
            <option>全部狀態</option>
            <option v-for="status in statusTypes" :key="status.complaintStatusId" :value="status.complaintStatusId">{{ status.complaintStatusName }}</option>
            <!-- <option>待處理</option>
            <option>已受理</option>
            <option>處理中</option>
            <option>處理完成</option> -->
...
<div style="position: relative;">
  <Customer_System_Details v-if="selectedcase" 
                                :SelectCaseDetail="selectedcase" 
...
<Customer_System_NewCustomer ref="showCreate" 
                              @refresh="fetchCases"
/>
<Customer_system_editCustomer ref="Edit" @refresh="fetchCases"/>

<Record_System  v-if="selectedcase" :SelectCaseDetail="selectedcase"
                style = "position: absolute; left: 70%;"
                @ResetCaseStatus="GetRecordStatus(selectedcase)"
/>
...
console.log(api.get('/TimMovE_ComplaintCase_'));

const showCreate = ref(null);
const Edit = ref(null);
const selectedcase = ref(null);
const RecordStutas = ref(null);
const time = ref('')
const cases = ref([]);
...
const fetchCases = async () => {
  try {
    const res = await api.get('/TimMovE_ComplaintCase_');
    cases.value = res.data;
    console.log("資料已同步更新");
    if (selectedcase.value) {
      // 從剛抓回來的最新 cases 陣列中，找到跟目前開啟的案件編號一樣的那一筆
      const newData = cases.value.find(
        (c) => c.客訴紀錄編號 === selectedcase.value.客訴紀錄編號
      );

      if (newData) {
        // 把新資料重新賦值給 selectedcase，這時 Props 就會觸發子元件更新
        selectedcase.value.處理狀態 = newData.處理狀態
      }
    }
...
const GetRecordStatus = async(item) => {

      
      const res2 = await api.get(`/TimMovE_ComplaintCase_/${item.客訴紀錄編號}/getRecordStutas`)
      RecordStutas.value = res2.data;

      if(RecordStutas.value[RecordStutas.value.length-1].handleStatusId == 5){
        item.complaintStatusId = 4
      }
      else{
        item.complaintStatusId = RecordStutas.value[RecordStutas.value.length-1].handleStatusId
      }
      

      await api.patch(`/TimMovE_ComplaintCase_/${item.客訴紀錄編號}`,`${item.complaintStatusId}`// 更新為已受理
      ,{
      ...
      console.log(item)
      // console.log(RecordStutas.value[RecordStutas.value.length-1])
}
...
onMounted(async () => {
  const res = await api.get('/TimMovE_ComplaintCase_/status');
...
async function selectCase(item) {
  console.log(item);
  try {
      const res = await api.get(`/TimMovE_ComplaintCase_/${item.客訴紀錄編號}`); // 呼叫詳細 API
...
function Search() {
  ...
  api.get('/TimMovE_ComplaintCase_/search', { params: searchCase})
    ...
    .then(response => {
      ...
      if(cases.value.length > 0) {
        ...
        searchCase.客訴類型 = '全部類型';
        searchCase.案件嚴重度 = '全部嚴重度';
        searchCase.處理狀態 = '全部狀態';
      }
    })
    .catch(error => {
